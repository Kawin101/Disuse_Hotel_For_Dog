...
class UserForm(UserCreationForm):
    # A Form that use built-in UserCreationForm to handel user creation
...
class AuthForm(AuthenticationForm):
    # A Form that uses built-in AuthenticationForm to handel user auth
    username = forms.EmailField(max_length=254, required=True, widget=forms.TextInput(attrs={'placeholder': '*Email..'}))
...
class UserProfileForm(forms.ModelForm):
    # The Basic model-form for our user profile that extends Django user model.
